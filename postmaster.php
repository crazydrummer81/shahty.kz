...
// Отправка в Telegram
define('TELEGRAM_TOKEN', '1073702894:AAG7d-O3Y4Q0YM432rHoOQXVV68mftKAdx8');
define('TELEGRAM_CHATID', '336615395');
...
//Отправка на e-mail
$EMAIL = 'shahty.kz@gmail.com'; //  Ваш e-mail на который будут приходить заявки
$FROM = 'shahty.kz@gmail.com';  //  e-mail отправителя (иногда требуется указать разрешенный настройками e-mail)
$REPLY = 'shahty.kz@gmail.com';   //  e-mail для ответа 
...
switch (@$_POST['action']) { // рассматриваем варианты разных значений поля action
    ...
    case 'button_callback':
        sendmail("Заказ обратного звонка", render(
                        '<div><b>Имя:</b> '.stripinput($_POST['name']).'</div>'.
                        '<div><b>Телефон:</b> '.stripinput($_POST['tel']).'</div>'.
                        '<div><b>Филиал:</b> '.stripinput($_POST['filial']).'</div>'
        ),$EMAIL,$FROM,$REPLY);
        message_to_telegram("Заказ обратного звонка\n".
                            "\nИмя: *". $_POST['name'].
                            "*\nТелефон: *".$_POST['tel'].
                            "*\nФилиал: *".$_POST['filial']."*");
        exit('200');
    break;
    case 'button_gift':
        sendmail("Заказ сертификата на пробное посещение", render(
                        '<div><b>Имя:</b> '.stripinput($_POST['name']).'</div>'.
                        '<div><b>Телефон:</b> '.stripinput($_POST['tel']).'</div>'.
                        '<div><b>Филиал:</b> '.stripinput($_POST['filial']).'</div>'.
                        '<div><b>Подарок:</b> '.stripinput($_POST['gift']).'</div>'
                    ),$EMAIL,$FROM,$REPLY);
        message_to_telegram("Заказ сертификата на пробное посещение\n".
                            "\nИмя: *".$_POST['name'].
                            "*\nТелефон: *".$_POST['tel'].
                            "*\nФилиал: *".$_POST['filial'].
                            "*\nПодарок: *".$_POST['gift']."*");
    ...
    break;
    case 'button_buy':
        sendmail("Заказ абонемента", render(
                        '<div><b>Имя:</b> '.stripinput($_POST['name']).'</div>'.
                        '<div><b>Телефон:</b> '.stripinput($_POST['tel']).'</div>'.
                        '<div><b>Филиал:</b> '.stripinput($_POST['filial']).'</div>'.
                        '<div><b>Хочет:</b> '.stripinput($_POST['gift']).'</div>'
                    ),$EMAIL,$FROM,$REPLY);
        message_to_telegram("Заказ абонемента\n".
                            "\nИмя: *".$_POST['name'].
                            "*\nТелефон: *".$_POST['tel'].
                            "*\nФилиал: *".$_POST['filial'].
                            "*\nХочет: *".$_POST['gift']."*");
